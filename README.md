...
（**If you're using a proxy to access OpenAI, add your proxy server address to the openai.api_base variable.**）
...
### Process Overview  
**Facial Capture** -> **Emotion Analysis** -> **Response Generation** -> **Voice Output**

### 1. **Facial Emotion Recognition**  
   - The code implements real-time facial emotion analysis using the `DeepFace` library, with emotions smoothed using a sliding window to reduce short-term fluctuations and enhance stability.

### 2. **Response Engine**  
   - `OpenAI GPT-3` is used to generate personalized conversation content based on the user's emotions. Dynamic prompts are generated to adjust the conversation style, ensuring that the replies match the user's emotional state.

### 3. **Voice Interaction**  
   - Text generated by the response engine is converted to speech using `pyttsx3`, with speech rate and volume adjusted based on the recognized emotions.

### 4. **Real-time Performance**  
   - Multi-threading is employed to separate tasks such as facial capture, emotion analysis, and speech output, ensuring real-time performance and minimizing response delay.

### 5. **User Data Management**  
   - Local storage is used to record and manage user interaction data, enabling personalized conversation generation based on the user's history.


## Our advantages
### 1.We use the cutting-edge DeepFace library for sentiment analysis.
#### Why we choose Deepface for my project?
a. **Efficient facial emotion recognition**: DeepFace provides efficient facial emotion recognition capabilities, able to process video streams in real-time and accurately identify emotional states, meeting the facial emotion recognition accuracy requirement (≥90%).

b. **Multimodal support**: DeepFace can easily integrate with other emotion analysis models, effectively complementing voice emotion recognition and improving the overall system performance.

c. **Fast response and low latency**: DeepFace optimizes the facial recognition algorithm, supporting rapid emotion recognition processing and meeting real-time requirements.
...
b. **Efficient response generation**: GPT-3.5-Turbo can quickly generate high-quality speech responses, meeting low latency requirements.
...
#### Why we choose OpenAI's GPT-3.5-Turbo large language model for my project?
a. **Lightweight and efficient**: pyttsx3 is a lightweight text-to-speech library that can quickly generate speech output, meeting low latency requirements.

b. **Cross-platform support**: pyttsx3 supports cross-platform operation (Windows, macOS, Linux), making it easy to deploy the speech interaction system on various devices.
...
### 2. **User Data Management for Personalized Dialogue**: By serializing and deserializing user data in JSON format, historical emotional data is utilized to support personalized dialogue management.

### 3. **Emotion-Based Dialogue Generation**: Combining OpenAI GPT-3 model and emotion analysis results to improve the emotional relevance of the generated dialogue.

###  4. **Ensuring Synchronization Between Speech Output and Emotional Changes**: By setting a cooldown time for speech playback, the frequency of emotion analysis is controlled, ensuring synchronization between speech output and emotional shifts.



## Subsequent improvements and upgrades
### 1.Optimize efficiency with model quantization, lightweight models, and caching for common dialogs and audio.

### 2.Use a more advanced speech model API to generate a more cartoon-like voice (I am using pyttsx3 because I can't afford a premium platform subscription 😭).

### 3.Utilize asynchronous programming or multithreading to improve image processing and emotion analysis speed, avoiding main thread blocking.

### 4.Enhance logging mechanisms, categorizing and capturing different types of errors while implementing API failure retry mechanisms.

### 5.Dynamically adjust AI's personality and conversation content based on user feedback, adding customizable user preferences.

### 6.Integrate multimodal emotion recognition to improve accuracy and robustness.

### 7.Modify speech tone and timbre based on emotions to enhance user experience.

### 8.Strengthen user data security and privacy protection, ensuring compliance with relevant regulations.


## Contributing 
### You are welcome to do continuous optimisation! 😊
